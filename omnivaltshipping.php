...
class OmnivaltShipping extends CarrierModule
{
    ...
    foreach ($this->_carriers as $key => $value) {
      ...
      if ($carrier->add()) {
        ...
        Configuration::updateValue($value . '_reference', $carrier->id);
          ...
          $carrierCreateErrors = true;
      }
    }
    if ($carrierCreateErrors) {
        return !$carrierCreateErrors;
    }
    return TRUE;
  }
  ...
  */
  ...
  public function getContent()
  {
    ...
    {
        $api_url = strval(Tools::getValue('omnivalt_api_url'));
        $api_user = strval(Tools::getValue('omnivalt_api_user'));
        $api_pass = strval(Tools::getValue('omnivalt_api_pass'));
        $send_off = strval(Tools::getValue('omnivalt_send_off'));
        $cod = strval(Tools::getValue('omnivalt_cod'));
        $bank_account = strval(Tools::getValue('omnivalt_bank_account'));
        $company = strval(Tools::getValue('omnivalt_company'));
        $pick_up_time_start = strval(Tools::getValue('omnivalt_pick_up_time_start'));
        $pick_up_time_finish = strval(Tools::getValue('omnivalt_pick_up_time_finish'));
        if(empty($api_url) || empty($api_user) || empty($api_pass) || empty($send_off) || empty($bank_account) || empty($company) || empty($pick_up_time_start) || empty($pick_up_time_finish))
          $output .= $this->displayError($this->l('All fields required'));
        else
        {
          Configuration::updateValue('omnivalt_api_url', $api_url);
          Configuration::updateValue('omnivalt_api_user', $api_user);
          Configuration::updateValue('omnivalt_api_pass', $api_pass);
          Configuration::updateValue('omnivalt_send_off', $send_off);
          Configuration::updateValue('omnivalt_cod', $cod);
          Configuration::updateValue('omnivalt_bank_account', $bank_account);
          Configuration::updateValue('omnivalt_company', $company);
          Configuration::updateValue('omnivalt_pick_up_time_start', $pick_up_time_start);
          Configuration::updateValue('omnivalt_pick_up_time_finish', $pick_up_time_finish);
...
public function displayForm()
  ...
  private function getTerminalsOptions($selected = '')
  {
  ...
  public function hookDisplayBeforeCarrier($params)
  {
    ...
    $this->context->smarty->assign(array(
        ...
        'parcel_terminals' => $this->getTerminalsOptions(),
    ));
  ...
  public static function get_tracking_number($id_order){
                    ...
                    <item_list>
                      ...
                      $xmlRequest .= '
                       <item service="'.$service.'" >
                          ...
                          <!--Optional:-->
 						<returnAddressee>
							<person_name>Pet24 - UAB Future Retail</person_name>
							<phone>+37061141447</phone>
							<address postcode="06118" deliverypoint="Vilnius" country="LT" street="Laisvės pr. 125" />
						</returnAddressee>
                       </item>';
                       endfor;
                       $xmlRequest .= '
  ...
  {
      if (!count(self::$t)) {
          Module::getInstanceByName('omnivaltshipping');
      }
    $barcodes = array();;
    $errors = array();
        ...
        "Content-length: ".strlen($request),
            ...
            if ($xmlResponse === false) {
                ...
                if (strlen(trim($xmlResponse)) > 0) {
                  ...
                  if (!is_object($xml)) {
                    $errors[] = self::$t['Response is in the wrong format'];
                  }
