    ...
    if (is_array($services) && count($services) && ($address['iso_code_2'] == 'LT' || $address['iso_code_2'] == 'LV' || $address['iso_code_2'] == 'EE') ){
      foreach ($services as $key=>$service){
        ...
        $cost = $this->config->get('omnivalt_'.$service.'_price');
                //Adition for Latvia Estonia pricing
...
<select name="omnivalt_parcel_terminal" id="omnivalt_parcel_terminal" class="form-control form-inline input-sm" style="width: 80%; display: inline;"
...
*/
          $cabine_select = '<script>$( "input[name=shipping_method]" ).focus(function() { $( this ).blur(); });$(document).ready(function(){$(\'input[id^=fake\]\').hide();});</script>
<select name="omnivalt_parcel_terminal" id="omnivalt_parcel_terminal" class="form-control form-inline input-sm" style="width: 80%; display: inline;"
onchange="thenum = $(this).val().match(/\d+$/)[0];$(\'input[id$=parcel_terminal_\'+thenum+\']\').prop(\'checked\', true);" 
>';
